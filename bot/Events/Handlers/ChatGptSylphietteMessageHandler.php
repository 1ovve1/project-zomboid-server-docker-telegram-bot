...
class ChatGptSylphietteMessageHandler implements HandlerInterface
{
  /**
  ...
  public function __invoke(mixed ...$params): void
  {
    foreach ($params as $message) {
      $choice = $this->chatGpt->answerWithoutUserId("сгенерируй милую фразу, в которой ты в роли стеснительной девочки эльфийки коротко желаешь {$message}. Без слов о том, что ты искуственный интеллект. Не говоря о том, что ты стеснительная девочка эльфийка. Фараз должна быть адресована одному человеку. В предложении должна чувствоваться скромность и стеснение, а самое главное отсутствовать навязчивость");

      $content = $choice->message->content;

      try {
        $imagePath = $this->imageReslover->getRandomPicturePath("/sylphiette");
